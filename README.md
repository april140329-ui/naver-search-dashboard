...
8. **민원 감정 분석 전용 페이지** (`민원 분석`):
   - 수집한 글을 **8개 민원 유형**(소음, 악취·환경오염, 쓰레기·청소, 교통·주차, 안전·위험, 시설·보수, 불친절·응대, 처리지연·미조치)으로 자동 분류
   - 부정 감정 강도 + 긴급·반복 표현을 합산한 **심각도(심각/보통/경미)** 산출
   - 유형별 분포·심각도 구성, 유형별 주요 표현, 심각도 높은 민원 글 목록 제공
9. **지역 분포 분석**:
...
│   │   └── complaint.py           # 민원 유형 분류 및 심각도(부정 강도+긴급 표현) 산출
│   ├── components/
...
│   │   └── complaint_view.py      # 민원 유형 분포, 심각도 구성, 심각 민원 글 목록
